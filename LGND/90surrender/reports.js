...
function report() {
	...
	let vehiclecolor = document.getElementById('vehiclecolor').value;
	let vehicledesc = document.getElementById('vehicledesc').value;
	let vehiclereg = document.getElementById('vehiclereg').value;
    if (plate || vehiclecolor || vehicledesc || vehiclereg) buffer.push('');
	if (plate || vehiclecolor || vehicledesc || vehiclereg) buffer.push(`[VEHICLE DESCRIPTION]:`);
	
    if (vehicledesc || vehiclecolor) vehicledesc = `was a ${vehiclecolor} in colored ${vehicledesc}${(plate ? ' (PLATE: ' + plate + ')' : '')}`;
	if (plate || vehiclecolor || vehicledesc || vehiclereg) buffer.push(`The vehicle parked outside ${vehicledesc}. The vehicle was registered to an individual named ${vehiclereg}.`);
	if (plate || vehiclecolor || vehicledesc || vehiclereg) buffer.push('');
