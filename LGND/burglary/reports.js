...
function report() {
    ...
    if (location) buffer.push(`Whilst out on routine patrol, we responded to a 10-31 Dispatch call of a reported burglary near the area of ${location}.`);
    
	buffer.push(`Upon arriving near the immediate dispatched area, we began looking for an individual that might be lurking around the area acting suspicious.`);
    buffer.push('');
    ...
    let breachInfo = {
        ...
        'No': {
            text: 'While still looking for any potential suspects, we did manage to witness an individual lockpick into one of the houses in the near area. Once they were in there, we waited outside of the house until the suspect came outside.',
        }
    };
